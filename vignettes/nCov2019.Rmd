...
To provide convenient access to epidemiological data on the coronavirus outbreak, we developed an R package, nCov2019 (https://github.com/GuangchuangYu/nCov2019). Besides detailed basis statistics, it also includes information about vaccine development and therapeutics candidates. We redesigned the function plot() for geographic maps visualization and provided a  interactive shiny app. These analytics tools could be useful in informing the public and studying how this and similar viruses spread in populous countries.
...
The query resujlt of global status will contain a data frame with 21 types of statistic, which have detail explanation on the bottom of this documents.
```{r}
x = res$global
names(x$table) 
```
...
Here is the example for operate latest data.
```{r}
x = res$latest
...
To subset latest data could be easily done by using `[`. `x["Global"]` or x`["global"]` will return the data frame for all countries  and users could determine a specific country, such as:
```{r}
...
Historical data is useful in retrospective analysis or to establish predictive models,  the operation is similar as  latest data, user could determine the data frame for all country or some specific countries: 
